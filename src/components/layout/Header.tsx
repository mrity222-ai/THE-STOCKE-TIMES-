...
export const Header: React.FC<HeaderProps> = ({ activeTab, onNavigate }) => {
  ...
  return (
    <header className="sticky top-0 z-[200] bg-white transition-all duration-200 overflow-visible" style={{
      ...
      <div className="bg-white border-b border-[#E2E8F0] select-none">
        <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 min-h-16 sm:min-h-20 py-2 flex items-center justify-between gap-1.5 sm:gap-3 lg:gap-5">
          ...
          <div 
            ...
            className="flex flex-col cursor-pointer group shrink-0 min-w-fit"
          >
            <span className="text-[11px] font-extrabold uppercase tracking-widest text-slate-500 font-sans block -mb-1">
              THE
            </span>
            <span className="text-xl sm:text-3xl font-extrabold text-[#0B1F33] font-serif tracking-tight leading-none group-hover:text-[#155EEF] transition-colors">
              STOCK TIMES
            </span>
          ...
          <form 
            ...
            className="hidden md:flex items-center relative flex-1 max-w-md min-w-0"
          >
            <input
              type="text"
          ...
          <div className="flex items-center gap-1 sm:gap-2.5 shrink-0">
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-2 bg-[#0B1F33] hover:bg-[#0B1F33]/90 text-white text-xs sm:text-sm font-extrabold px-3.5 py-2.5 rounded-xl transition-all cursor-pointer shadow-sm whitespace-nowrap"
            >
      ...
      {/* ==========================================
         ...
         ========================================== */}
      <div className="bg-[#0B1F33] hidden lg:block border-b border-slate-800 text-white select-none relative z-[230] overflow-visible">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-12 flex items-center justify-between gap-3 relative overflow-visible" ref={megaMenuRef}>
          
          <nav className="flex items-center gap-1 font-sans text-sm font-semibold tracking-wide min-w-0 overflow-visible py-2">
            {primaryNavLinks.map((link) => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-2 rounded-xl transition-colors cursor-pointer relative flex items-center text-[10px] xl:text-[11px] whitespace-nowrap border ${
                    isActive ? 'text-white font-extrabold bg-white/10 border-white/15' : 'text-slate-200 hover:text-white hover:bg-white/5 border-transparent'
                  }`}
                >
                  <span>{link.label}</span>
                  {/* Active Indicator 3px Green Line */}
                  {isActive && (
                    <span className="absolute -bottom-2 left-3 right-3 h-[3px] bg-[#16A34A] rounded-t" />
                  )}
                </button>
              );
            })}
            ...
            <div className="relative">
              <button
                ...
                className={`px-3 py-2 rounded-xl transition-colors cursor-pointer flex items-center gap-1.5 text-[10px] xl:text-[11px] whitespace-nowrap border ${
                  toolsMegaOpen || activeTab.includes('calculator') || activeTab.includes('vs') || activeTab.includes('tools')
                    ? 'text-white font-extrabold bg-white/10 border-white/15'
              ...
              >
                ...
                {(activeTab.includes('calculator') || activeTab.includes('vs')) && (
                  <span className="absolute -bottom-2 left-3.5 right-3.5 h-[2px] bg-[#16A34A]" />
                )}
              </button>

              {/* TOOLS MEGA-MENU DROPDOWN */}
              {toolsMegaOpen && (
                <div className="absolute top-full right-0 left-auto mt-2 w-[560px] max-w-[calc(100vw-32px)] bg-white text-[#111827] rounded-2xl shadow-2xl border border-[#E2E8F0] p-6 z-[500] grid grid-cols-2 gap-6 animate-in fade-in duration-150">
          ...
          <div className="hidden xl:flex text-[11px] text-slate-400 font-mono items-center gap-2 shrink-0">
      ...
      {/* ==========================================
         ...
         ========================================== */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-[#E2E8F0] px-4 pt-4 pb-8 space-y-6 shadow-2xl animate-in slide-in-from-top-2 duration-200 relative z-[240]">
          
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex flex-col">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">THE</span>
              <span className="text-xl font-extrabold text-[#0B1F33] font-serif">STOCK TIMES</span>
            </div>
